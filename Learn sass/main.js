...
// let counter = setInterval(contUp, 1);
...
let div = document.querySelector(".expreiment");

if (window.localStorage.getItem("color")) {
    // div color
    div.style.backgroundColor = window.localStorage.getItem("color");

    // remove active frome all lis
    lis.forEach((li) => {
        li.classList.remove("active");
    });

    // add active class to the color li
    document
...
lis.forEach((li) => {
    li.addEventListener("click", (e) => {
        ...
        // add active class in click
        e.currentTarget.classList.add("active");
        //add currunt color in locale Storage
...
if (window.sessionStorage.getItem("user-name")) {
    myinput.value = window.sessionStorage.getItem("user-name");
}
myinput.onblur = function () {
    window.sessionStorage.setItem("user-name", this.value);
};
